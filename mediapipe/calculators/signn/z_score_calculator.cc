...
namespace mediapipe{
    ...
    class ZScoreCalculatorOneHand : public CalculatorBase {
        public:
        ZScoreCalculatorOneHand(){};
        ~ZScoreCalculatorOneHand(){};
        ...
        ::mediapipe::Status Process(CalculatorContext* cc){
            ...
            // send to output
            std::unique_ptr<NormalizedLandmarkList> output_stream_collection = std::make_unique<NormalizedLandmarkList>(zscores);
            cc -> Outputs().Tag(NormalizedLandmarks).Add(output_stream_collection.release(), cc->InputTimestamp());
    ...
    REGISTER_CALCULATOR(ZScoreCalculatorOneHand);
}