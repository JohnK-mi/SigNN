...
namespace mediapipe{
    ...
    class OneHandGateCalculator: public CalculatorBase {
    public:
    OneHandGateCalculator(){};
    ~OneHandGateCalculator(){};
    ...
    ::mediapipe::Status Process(CalculatorContext* cc){
        ...
        for(int i = 0; i < hand.landmark_size(); i++){
            if(hand.landmark(i).x() != 0){
                all_zero = false;
                break;
            }
        }
        hand_history.add(!all_zero);
        std::vector<bool> v = hand_history.get();
        int trues = 0;
        for(int i = 0; i < v.size(); i++){
            if(v[i] == true){
    ...
    REGISTER_CALCULATOR(OneHandGateCalculator);
